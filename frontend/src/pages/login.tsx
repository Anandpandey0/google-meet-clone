    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (response) {
          ...
          if (status === 404) {
            alert("User not found:" + data);
          } else if (status === 403) {
            alert("Incorrect password:" + data);
          } else {
            alert("Other authentication error:" + data);
          }
        } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen">
      <div className="lg:h-min-[70vh] h-fit w-full flex-wrap lg:w-[30vw] border-2 borde-solid   p-4 px-12 text-center flex flex-col  ">
        ...
        <h2 className=" text-2xl mt-4 font-medium">{showLogin && "Sign in"}</h2>
        <h3 className="pt-2">Use your Google Account</h3>
        {showLogin ? (
          <>
            <FloatingInput
              type="text"
            ...
            <a
              href="/forgot"
              className="text-left  mt-2 text-blue-800 font-medium"
            >
              Forgot mail?
            </a>
            <p className="text-left text-sm mt-12">
            ...
            <div className="flex justify-between my-auto flex-wrap">
              ...
              <button
                className="py-2 bg-blue-600 text-white px-6 rounded-md"
                onClick={LoginUser}
              >
                Login
              </button>
          ...
          <div>
            ...
            <div className="flex w-full items-center text-center justify-between  mt-8">
              ...
              <h3 className="font-semibold">{userInfo?.email}</h3>
            </div>

            <button className="bg-blue-600 p-2 mt-8" onClick={signoutHandler}>
              Sign Out
            </button>
          </div>
        )}
